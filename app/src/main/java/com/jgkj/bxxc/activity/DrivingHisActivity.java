...
import com.jgkj.bxxc.adapter.DrivingHisAdapter;
...
public class DrivingHisActivity extends Activity implements View.OnClickListener,
        SwipeRefreshLayout.OnRefreshListener, RefreshLayout.OnLoadListener {
    private ListView listView;
    private DrivingHisAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.drivinghis);
    ...
    private void getHis(String page) {
        Log.i("百信学车","陪练参数" + "uid=" + uid + "   page=" + page);
        OkHttpUtils
                .post()
                .url(Urls.pjShowApply)
                .addParams("uid", uid + "")
                .addParams("page", page)
                .build()
                .execute(new StringCallback() {
                    ...
                    public void onError(Call call, Exception e, int i) {
                        Toast.makeText(DrivingHisActivity.this, "发送失败", Toast.LENGTH_LONG).show();
                    }
                    @Override
        ...
        switch (swipehis.getTag().toString()) {
            case "refresh":
                if (action.getCode() == 200) {
                    ...
                    adapter = new DrivingHisAdapter(DrivingHisActivity.this, list,1,token,uid);
                    listView.setAdapter(adapter);
                } else {
                    Toast.makeText(DrivingHisActivity.this, action.getReason(), Toast.LENGTH_SHORT).show();
                }
                break;
            case "onload":
                if (action.getCode() == 200) {
    ...
    private void getNotHis(String page) {
        Log.i("百信学车","陪练未到参数" + "uid=" + uid + "   page=" + page);
        OkHttpUtils
                .post()
                .url(Urls.pjShowApplyNotTo)
                .addParams("uid", uid + "")
                .addParams("page", page)
                .build()
                .execute(new StringCallback() {
    ...
    public void createSureDialog() {
        sureDialog = new Dialog(DrivingHisActivity.this, R.style.ActionSheetDialogStyle);
        // 填充对话框的布局
        sureView = LayoutInflater.from(DrivingHisActivity.this).inflate(
        ...
        switch (swipehis.getTag().toString()) {
            case "refresh":
                if (action.getCode() == 200) {
                    list.clear();
                    list.addAll(action.getResult());
                    adapter = new DrivingHisAdapter(DrivingHisActivity.this, list,0,token,uid);
                    listView.setAdapter(adapter);
                } else {
                    Toast.makeText(DrivingHisActivity.this, action.getReason(), Toast.LENGTH_SHORT).show();
                }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.radio_btn01:
                ...
                adapter = new DrivingHisAdapter(DrivingHisActivity.this, list,1,token,uid);
                listView.setAdapter(adapter);
                tips.setText("温馨提示：这里显示的是你参加了陪练的时间段。");
            ...
            case R.id.radio_btn02:
                ...
                adapter = new DrivingHisAdapter(DrivingHisActivity.this, list,0,token,uid);
                listView.setAdapter(adapter);
                tips.setText("温馨提示：这里显示的是你预约的但是没有去陪练的时间段。");
            ...
            case R.id.baoming:
                if (baoming.isEnabled()) {
                    ...
                    Toast.makeText(DrivingHisActivity.this, "您暂时不能报名哦！", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.dialog_sure:
                //dialog = ProgressDialog.show(DrivingHisActivity.this, null,"申请中...");
                //applykaoshi(uid + "");
                //sureDialog.dismiss();
                break;
...
//                            dialog.dismiss();
//                        }
//                        Toast.makeText(DrivingHisActivity.this, "请检查网络", Toast.LENGTH_SHORT).show();
//                    }
//                    @Override
//                    public void onResponse(String s, int i) {
//                        if(dialog.isShowing()){
...
//                        }
//                        Toast.makeText(DrivingHisActivity.this, userInfo.getReason(), Toast.LENGTH_SHORT).show();
//                    }
//                });
//    }
